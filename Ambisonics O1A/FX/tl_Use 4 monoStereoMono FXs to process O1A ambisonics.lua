-- @description Set up four instances of FX to process channels 1-4
-- @version 1.0
...
--[[ @details Utility for FX processing of first order ambisonics using four FXs
  ...
  3) All parameters of second to fourth instances are linked to the parameters of the first
  4) Input and output pins are set so that
      - 1st FX instance processes channel 1
      - 2nd FX instance processes channel 2
      - 3rd FX instance processes channel 3
      - 4th FX instance processes channel 4
    All instances pass audio to left as well as right channel of the plugin, 
    and mixes the two return channels back to the one destination channel in output routing
...
function getSymbols(aString)
  ...
  -- If path has space, it need to be wrapped with quotation marks again (not very elegant, but works)
...
function tl_AmbiProcess4ChannelsUsingFourMonoToStereoToMonoFXs()
  ...
  local link_strFX3 = ( fx+2 )..':'..-2
  local link_strFX4 = ( fx+3 )..':'..-3
  ...
  local FXchunk_3   = ModNewFxChunk( FXchunk, fxGUID, true,   track, fx, true,   fxType, numParams, link_strFX3 )
  local FXchunk_4   = ModNewFxChunk( FXchunk, fxGUID, true,   track, fx, true,   fxType, numParams, link_strFX4 )
  ...
  for i = 0, inputPins - 1 do
  	...
  	reaper.TrackFX_SetPinMappings(track, fx + 2, 0, i, 0, 0)
  	reaper.TrackFX_SetPinMappings(track, fx + 2, 1, i, 0, 0)
  	reaper.TrackFX_SetPinMappings(track, fx + 3, 0, i, 0, 0)
  	reaper.TrackFX_SetPinMappings(track, fx + 3, 1, i, 0, 0)
  end
  
  -- Route channel 1 to stereo FX and then back to channel 1
  reaper.TrackFX_SetPinMappings(track, fx,     0, 0, 1, 0)
  reaper.TrackFX_SetPinMappings(track, fx,     0, 1, 1, 0)
  reaper.TrackFX_SetPinMappings(track, fx,     1, 0, 1, 0)
  reaper.TrackFX_SetPinMappings(track, fx,     1, 1, 1, 0)
  
  -- Route channel 1 to stereo FX and then back to channel 1
  reaper.TrackFX_SetPinMappings(track, fx + 1,     0, 0, 2, 0)
  reaper.TrackFX_SetPinMappings(track, fx + 1,     0, 1, 2, 0)
  reaper.TrackFX_SetPinMappings(track, fx + 1,     1, 0, 2, 0)
  reaper.TrackFX_SetPinMappings(track, fx + 1,     1, 1, 2, 0)
  
  -- Route channel 1 to stereo FX and then back to channel 1
  reaper.TrackFX_SetPinMappings(track, fx + 2,     0, 0, 4, 0)
  reaper.TrackFX_SetPinMappings(track, fx + 2,     0, 1, 4, 0)
  reaper.TrackFX_SetPinMappings(track, fx + 2,     1, 0, 4, 0)
  reaper.TrackFX_SetPinMappings(track, fx + 2,     1, 1, 4, 0)
  
  -- Route channel 1 to stereo FX and then back to channel 1
  reaper.TrackFX_SetPinMappings(track, fx + 3,     0, 0, 8, 0)
  reaper.TrackFX_SetPinMappings(track, fx + 3,     0, 1, 8, 0)
  reaper.TrackFX_SetPinMappings(track, fx + 3,     1, 0, 8, 0)
  reaper.TrackFX_SetPinMappings(track, fx + 3,     1, 1, 8, 0)
...
-- Finally!
tl_AmbiProcess4ChannelsUsingFourMonoToStereoToMonoFXs()